...
module.exports = (upload, deleteFile) => {
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      const sortField = ["name", "slug", "sort_order", "id"].includes(sort)
        ? sort
        : "id";
      const sortOrder = order === "desc" ? "DESC" : "ASC";
      const offset = (currentPage - 1) * 20;
      const products = await pool.query(
        `SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id ${where} ORDER BY ${sortField} ${sortOrder} LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
        [...params, 20, offset],
      );
      ...
      const totalPages = Math.ceil(
        parseInt(totalResult.rows[0].count, 10) / 20,
      );
  ...
  router.get("/create", async (req, res) => {
    ...
    const allPackageTypes = await pool.query(
      "SELECT * FROM package_types ORDER BY name",
    );
    const allDirections = await pool.query(
      "SELECT * FROM directions ORDER BY sort_order",
    );
    const body = await renderBody(
      "admin/products/form.ejs",
      {
        product: null,
        ...
        allDirections: allDirections.rows,
        selectedDirections: [],
        errors: [],
      },
      req,
  ...
  router.post("/create", requireFields("name"), async (req, res) => {
    try {
      ...
      if (properties) properties = DOMPurify.sanitize(properties);

      if (!slug || !slug.trim()) slug = await generateUniqueSlug(slugify(name));
      else {
        slug = slug.trim();
        const ex = await pool.query("SELECT id FROM products WHERE slug = $1", [
          slug,
        ]);
        if (ex.rows.length > 0) slug = await generateUniqueSlug(slug);
      }
      ...
      if (errors.length > 0) {
        ...
        const body = await renderBody(
          "admin/products/form.ejs",
          {
            ...
            selectedPackageTypes: [],
            allDirections: allDirections.rows,
            selectedDirections: req.body.directions || [],
          },
          req,
        );
      ...
      let catIds = Array.isArray(req.body.categories)
        ? req.body.categories
        : req.body.categories
          ? [req.body.categories]
          : [];
      ...
      let pkgIds = Array.isArray(req.body.package_type_ids)
        ? req.body.package_type_ids
        : req.body.package_type_ids
          ? [req.body.package_type_ids]
          : [];
      const volumesMap = {};
      for (const key in req.body) {
        if (key.startsWith("volume_"))
          volumesMap[key.split("_")[1]] = req.body[key]?.trim() || "";
      }
      for (const ptid of pkgIds) {
        await pool.query(
          "INSERT INTO product_package_types (product_id, package_type_id, volume) VALUES ($1,$2,$3)",
          [productId, ptid, volumesMap[ptid] || ""],
        );
      }

      let dirIds = Array.isArray(req.body.directions)
        ? req.body.directions
        : req.body.directions
          ? [req.body.directions]
          : [];
      for (const dirId of dirIds) {
        if (dirId)
          await pool.query(
            "INSERT INTO product_directions (product_id, direction_id) VALUES ($1,$2)",
            [productId, dirId],
          );
      }
  ...
  router.get("/:id/edit", async (req, res) => {
    try {
      ...
      const selCatRes = await pool.query(
      ...
      const selPkgRes = await pool.query(
      ...
      const selDirRes = await pool.query(
        "SELECT direction_id FROM product_directions WHERE product_id = $1",
        [id],
      );
      ...
      const body = await renderBody(
        "admin/products/form.ejs",
        {
          ...
          selectedCategories: selCatRes.rows.map((r) => r.category_id),
          allPackageTypes: allPackageTypes.rows,
          selectedPackageTypes: selPkgRes.rows,
          allDirections: allDirections.rows,
          selectedDirections: selDirRes.rows.map((r) => r.direction_id),
          errors: [],
        },
        req,
      );
  ...
  router.post("/:id/edit", requireFields("name"), async (req, res) => {
    try {
      ...
      if (!slug || !slug.trim())
        slug = await generateUniqueSlug(slugify(name), id);
      else {
        slug = slug.trim();
        ...
        if (ex.rows.length > 0) slug = await generateUniqueSlug(slug, id);
      }
      ...
      if (errors.length > 0) {
        ...
        const body = await renderBody(
          "admin/products/form.ejs",
          {
            product: { ...product.rows[0], ...req.body, slug },
            errors,
            action: `/admin/products/${id}/edit`,
            categories: allCategories,
            selectedCategories: selCatRes.rows.map((r) => r.category_id),
      ...
      await pool.query(
        "DELETE FROM product_package_types WHERE product_id = $1",
        [id],
      );
      let pkgIds = Array.isArray(req.body.package_type_ids)
        ? req.body.package_type_ids
      ...
      for (const ptid of pkgIds) {
        await pool.query(
          "INSERT INTO product_package_types (product_id, package_type_id, volume) VALUES ($1,$2,$3)",
          [id, ptid, volumesMap[ptid] || ""],
        );
      }

      await pool.query("DELETE FROM product_directions WHERE product_id = $1", [
        id,
      ]);
      let dirIds = Array.isArray(req.body.directions)
        ? req.body.directions
