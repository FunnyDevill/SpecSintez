...
function renderProduct(product, container) {
            ...
            <div class="gallery-thumbs-vertical">
                ${product.images.map(img => `
                    <div class="gallery-thumb-vertical" data-image="${img.image_url}">
                        <img src="${img.image_url}" alt="Миниатюра" loading="lazy" onerror="this.classList.add('image-hidden')">
                    </div>
                `).join('')}
            </div>
        </div>
            ...
            <div class="package-icons-list-compact">
                ${product.package_types.map(pt => `
                    <div class="package-icon-item-compact">
                        <img src="${pt.icon_url}" alt="${window.escapeHtml(pt.name)}" onerror="this.classList.add('image-hidden')">
                ...
                <div class="options-list">
                    ${product.options.map(opt => `
                        <div class="option-item" data-option-id="${opt.id}" data-image="${opt.image_url || ''}">
                            ${opt.image_url ? `<img src="${opt.image_url}" class="option-icon" onerror="this.classList.add('image-hidden')">` : ''}
   ...
   // ========== БЛОК ОТРАСЛЕЙ С ИСПОЛЬЗОВАНИЕМ ОБЩЕЙ ФУНКЦИИ ИКОНОК ==========
   let industriesHtml = '';
            ...
            <div class="product-industries">
                ...
                <div class="product-industries__list">
                    ${product.categories.map(cat => `
                        <a href="/catalog.html?category=${cat.slug}" class="product-industry-item">
        ...
        <article class="product-detail">
            ...
            <div class="product-media-row">
                <div class="product-main-image">
                    <img src="${currentImage}" alt="${window.escapeHtml(product.name)}" id="mainProductImage" onerror="this.classList.add('image-hidden')">
                </div>
                ${galleryHtml}
            </div>
            ${packageIconsHtml}
   ...
   // Для миниатюр и иконок опций – то же самое:
   document.querySelectorAll('.gallery-thumb-vertical img, .option-icon, .package-icon-item-compact img').forEach(img => {
