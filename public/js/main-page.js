document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (industriesGrid) {
        try {
            ...
            // Берём только корневые категории (parent_id === null) – это отрасли
...
// Функция рендера новостей (без изменений)
    ...
    for (const item of newsList) {
        ...
        card.innerHTML = `
            <div class="news-card__image-placeholder">
                ${item.image_url ? `<img src="${item.image_url}" alt="${window.escapeHtml(item.title)}" style="width:100%;height:100%;object-fit:cover;">` : `
...
// Функция рендера товаров (без изменений)
    ...
    for (const prod of products) {
        ...
        card.innerHTML = `
            <div class="product-card__image-placeholder">
                ${prod.image_url ? `<img src="${window.escapeHtml(prod.image_url)}" alt="${window.escapeHtml(prod.name)}" onerror="this.style.display='none'; this.parentElement.classList.add('fallback')">` : `
