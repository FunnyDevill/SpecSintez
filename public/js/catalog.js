...
async function loadProductsBySearch(query) {
    productsContainer.innerHTML = '<div class="loading">Поиск точного совпадения...</div>';
    try {
        const res = await fetch(`/api/products?search=${encodeURIComponent(query)}`);
        if (!res.ok) throw new Error('Ошибка поиска');
        allProducts = await res.json();
        if (allProducts.length === 0) {
            productsContainer.innerHTML = '<div class="no-products">Товар с таким названием не найден</div>';
            removePagination();
        } else {
            // Показываем только точные совпадения (их будет 0 или 1, но на всякий случай массив)
    ...
    searchClear.addEventListener('click', function() {
        ...
        // Возвращаем стандартное поведение (первая корневая категория)
        if (allCategories.length > 0) {
            const roots = allCategories.filter(c => c.parent_id === null);
            if (roots.length) {
                roots.sort((a,b) => (a.sort_order||0) - (b.sort_order||0));
