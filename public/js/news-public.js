  ...
  container.innerHTML = `
        <article class="hero-news">
            <div class="hero-news__content">
                ...
                <h2>${window.App.escapeHtml(news.title)}</h2>
                <p>${window.App.escapeHtml(news.excerpt || "")}</p>
                <a href="/news/${news.slug}" class="hero-news__link">Подробнее →</a>
            </div>
            <div class="hero-news__image-placeholder">
                ${
                  news.image_url
                    ? `<img src="${news.image_url}" alt="${window.App.escapeHtml(news.title)}" class="cover-image">`
                    : `
  ...
  for (const item of newsList) {
    ...
    card.innerHTML = `
            <div class="news-card__image-placeholder">
                ${
                  item.image_url
                    ? `<img src="${item.image_url}" alt="${window.App.escapeHtml(item.title)}" class="cover-image">`
                    : `
